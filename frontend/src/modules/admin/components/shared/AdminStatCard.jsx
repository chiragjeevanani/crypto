...
export default function AdminStatCard({ label, value, change, icon: Icon, color = 'primary', delay = 0, path, onClick, className = '' }) {
    ...
    return (
        <motion.div
            ...
            className={`bg-surface border border-surface p-5 rounded-lg relative overflow-hidden group hover:border-primary/20 transition-all shadow-sm ${path || onClick ? 'cursor-pointer' : ''} ${className}`}
            onClick={onClick}
        >
            {path ? (
