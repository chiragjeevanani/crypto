...
export default function AdvertiserPanel() {
    ...
    React.useEffect(() => {
        const init = async () => {
            await Promise.all([
                loadModerationStats(),
                loadAdvertiserPosts(), // Fetch recent posts for the queue
                loadFinancials() // Fetch ground truth financial data
            ]);
            acknowledgeAds();
        };
        init();
    }, [loadAdvertiserPosts, loadModerationStats, acknowledgeAds, loadFinancials]);

    const advertiserStats = useMemo(() => {
        // Use backend ground truth for revenue
        const totalRevenue = financialStats?.promotionRevenue || 0;
            
        const totalReach = posts.reduce((acc, p) => acc + (p.participants || 0), 0);
        const reachDisplay = totalReach > 999999 ? (totalReach / 1000000).toFixed(1) + 'M' : 
                          totalReach > 999 ? (totalReach / 1000).toFixed(1) + 'K' : totalReach;

        return [
            { label: 'Pending Ads', value: moderationStats.ads, icon: Megaphone, trend: 'Requires Review', color: 'primary', path: '/admin/content?type=promotion' },
            { label: 'Active Reach', value: reachDisplay, icon: Users, trend: '↑ 12% (Est)', color: 'blue-500', path: '/admin/content?type=promotion' },
            { label: 'Conversion Rate', value: '3.8%', icon: Target, trend: '↑ 0.5% (Est)', color: 'emerald-500', path: '/admin/content?type=promotion' },
            { label: 'Total Revenue', value: `₹${totalRevenue.toLocaleString()}`, icon: CreditCard, trend: 'Verified: 85%', color: 'rose-500', path: '/admin/wallet?type=promotion' }
        ];
    }, [posts, moderationStats.ads, financialStats]);

    const filteredPosts = useMemo(() => {
        return posts.filter(p => (p.status || 'pending').toLowerCase() === statusFilter.toLowerCase());
    }, [posts, statusFilter]);
                    ...
                    {advertiserStats.map((stat) => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div className={`p-2.5 bg-bg rounded-xl border border-surface group-hover:text-primary transition-colors text-${stat.color}`}>
                            ...
                            <p className={`text-[9px] font-medium mt-2 uppercase tracking-tight ${stat.label === 'Total Revenue' ? 'text-emerald-500' : 'text-primary'}`}>
                                {stat.trend}
                            </p>
                        </div>
                    ))}
