import { Globe, Zap, RotateCcw, Save, AlertTriangle } from 'lucide-react';
    ...
    return (
        <div className="space-y-10 pb-20">
            <AdminPageHeader
                ...
                subtitle="Topology and node-level parameters."
                actions={
            ...
            <div className="bg-surface border border-surface rounded-lg p-6 space-y-8">
                <div className="space-y-4">
                    <p className="text-[10px] text-muted">Example parameters (read-only until engine support available):</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-[9px] font-bold uppercase text-muted">Node Count</label>
                            <input type="number" value={formData.nodeCount ?? 12} disabled className="w-full bg-bg border border-surface rounded-lg py-2 px-3 text-xs text-text" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[9px] font-bold uppercase text-muted">Sync Interval (s)</label>
                            <input type="number" value={formData.syncInterval ?? 30} disabled className="w-full bg-bg border border-surface rounded-lg py-2 px-3 text-xs text-text" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[9px] font-bold uppercase text-muted">RPC Endpoint</label>
                            <input type="text" value={formData.rpcEndpoint ?? 'https://node.example.com'} disabled className="w-full bg-bg border border-surface rounded-lg py-2 px-3 text-xs text-text" />
                        </div>
                    </div>
                </div>
                <div className="h-64 flex flex-col items-center justify-center text-center p-8 bg-bg/50 rounded-lg border border-dashed border-surface">
                    ...
                    <h4 className="text-sm font-bold text-text uppercase tracking-widest">Network Config</h4>
